...
int recorderCaptureTestMain(int, char**); // original dispatcher, renamed only in this target's CMake block
namespace
{
...
int main(int argc, char** argv)
{
    try
    {
        if (argc == 3 && std::string(argv[1]) == "--suite" && std::string(argv[2]) == "project-roundtrip") return runProjectTests();
        const auto existing = recorderCaptureTestMain(argc, argv); // preserves future suites merged from other rounds
        if (argc != 1) return existing;
        const auto project = runProjectTests(); return existing == 0 && project == 0 ? 0 : 1;
    }
