...
{
    std::uint64_t accepted=0,rejected=0,noops=0,undoSteps=0;std::array<int,14> operations{};
    try
    {
        std::mt19937_64 random(recorderEditPropertySeed);RecorderDocument d;const auto initial=fixture();ok(d.adopt(initial,{},{}));d.setSelection({fixedId(30)});const auto initialHash=hash(initial);const auto registry=initial.media;
        ...
        for(int base=0;base<recorderEditPropertyIterations;base+=1000)
        {
            ...
            const int count=(std::min)(1000,recorderEditPropertyIterations-base);
            for(int j=0;j<count;++j)
            {
                ...
                const int op=static_cast<int>(draw(14));++operations[static_cast<size_t>(op)];const auto chosen=ids.empty()?Id{}:ids[static_cast<size_t>(draw(ids.size()))];const auto other=ids.empty()?Id{}:ids[static_cast<size_t>(draw(ids.size()))];
                ...
                {
                    ++accepted;need(d.getProject().editRevision==before->editRevision+1,"revision monotonicity");
            ...
            if(base+count<recorderEditPropertyIterations){const auto state=d.getProject();ok(d.adopt(state,{},{}));}
        }
        std::cout<<"EditPropertyTests: PASS seed="<<recorderEditPropertySeed<<" iterations="<<recorderEditPropertyIterations<<" committed="<<accepted<<" rejected="<<rejected<<" no-op="<<noops<<" undo-steps="<<undoSteps<<" invariants=PASS undo-hash-selection=PASS redo=PASS\noperations:";
        for(const auto n:operations)std::cout<<' '<<n;std::cout<<'\n';return 0;
    }
    catch(const std::exception& e){std::cerr<<"EditPropertyTests: FAIL seed="<<recorderEditPropertySeed<<" iterations="<<recorderEditPropertyIterations<<" completed="<<(accepted+rejected+noops)<<" error="<<e.what()<<'\n';return 1;}
}
