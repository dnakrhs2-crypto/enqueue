                                                     ...
                                                     bool recording = false)
{
    ...
    const auto matches = [&](RecorderCommand command)
    { return key == juce::KeyPress::createFromDescription(bindings[command]); };
    if (matches(RecorderCommand::recordStop) && matches(RecorderCommand::playStop))
        return recording ? RecorderCommand::recordStop : RecorderCommand::playStop;
