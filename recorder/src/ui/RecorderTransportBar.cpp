...
RecorderTransportBar::RecorderTransportBar()
{
    ...
    for (auto* b : {&play, &stop, &beginning, &zoomOut, &zoomIn, &fit, &waveOut, &waveIn}) b->getProperties().set("recorderFontSize", 12.0f);
    time.setFont(recorderMonoFont(18, juce::Font::bold)); waveValue.setFont(recorderMonoFont(12));
    for (auto* l : {&zoom, &wave}) { l->setFont(recorderFont(12)); l->setColour(juce::Label::textColourId, Palette::dimText); }
    play.setColour(juce::TextButton::textColourOffId, Palette::meterGreen);
...
void RecorderTransportBar::paint(juce::Graphics& g)
{ g.fillAll(Palette::bar); g.setColour(Palette::line); g.fillRect(0, getHeight() - 1, getWidth(), 1); }
void RecorderTransportBar::resized()
{
    ...
    fit.setBounds(row.removeFromRight(58).reduced(2)); zoomIn.setBounds(row.removeFromRight(36).reduced(2)); zoomOut.setBounds(row.removeFromRight(36).reduced(2)); zoom.setBounds(row.removeFromRight(50));
    waveIn.setBounds(row.removeFromRight(36).reduced(2)); waveValue.setBounds(row.removeFromRight(44)); waveOut.setBounds(row.removeFromRight(36).reduced(2)); wave.setBounds(row.removeFromRight(46)); time.setBounds(row.reduced(4, 0));
}
}
