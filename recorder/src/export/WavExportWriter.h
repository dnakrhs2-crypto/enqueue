...
public:
    ...
    static Header makeHeader(std::uint32_t Fs, std::uint64_t samples);
    static Header readHeader(const void*, std::size_t);
    static Header inspect(const juce::File&); // also verifies physical file length
    WavExportWriter(juce::File finalFile, std::uint32_t Fs, Sample samples, FileIoFaultAdapter* = nullptr);
    ~WavExportWriter();
    WavExportWriter(const WavExportWriter&) = delete;
    void append(const float* mono, unsigned frames);
