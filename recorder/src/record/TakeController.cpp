...
public:
    TakeCameraMapper(CameraClockMapper& camera, ClockMapping c, std::int64_t start, unsigned rate)
        : camera(camera), clock(c), n0(start), Fs(rate), epoch(camera.snapshot()->epoch) {}
    std::int64_t map(const FrameStamp& frame) override
    {
        const auto snapshot = camera.snapshot();
        const auto qpc = snapshot && snapshot->epoch == epoch ? snapshot->timestampQpc(frame) : std::nullopt;
        if (!qpc) throw std::runtime_error("Camera clock generation/epoch changed or timestamp unavailable");
...
public:
    ...
    {
        mapper = std::make_unique<TakeCameraMapper>(*cameraClock, clock, n0, rate); Fs = rate; acceptedLength = std::move(length);
