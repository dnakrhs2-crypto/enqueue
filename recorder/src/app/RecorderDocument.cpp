...
// Exact int64 rounding; an out-of-range coordinate is reported before anything is replaced.
static juce::Result rescaleMarkers(RecorderProject& p, std::uint32_t oldFs, std::uint32_t newFs)
{
    if (!oldFs || !newFs || oldFs == newFs) return juce::Result::ok();
    try { for (auto& m : p.markers) m.sample = rescaleRound(m.sample, newFs, oldFs); }
    catch (const std::exception& e) { return juce::Result::fail(juce::String::fromUTF8(e.what())); }
    return juce::Result::ok();
}
