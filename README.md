...
설정 → 프로젝트 설정 → 단축키에서 명령 키를 변경할 수 있습니다. 현재 키는 메뉴와 단축키 탭에서 확인하세요. 아래 표는 기본값이며, 컴포넌트 자체 편집 키는 현재 고정입니다.
...
- **단축키(이 PC)**: 검색(기능명·설명·키 이름), 카테고리·상태 필터, 복수 키 추가·교체·삭제, 기능별·전체 기본값 복원. [학습]은 추가이며 키를 고른 뒤 [키 바꾸기]는 교체입니다. 후보와 충돌을 확인하고 [등록]합니다.
...
- **가져오기/내보내기**: UTF-8 `*.enqueue-shortcuts.xml`. 내보내기는 현재 전체 명령 키 목록, 가져오기는 변경·미지정·충돌 미리보기 후 설정 교체입니다. 실패하면 적용하지 않습니다. 큐 핫키와 오디오 장치는 포함하지 않습니다.
- **장치 입력**: F13~F24 지원. 넘패드는 JUCE가 구분하는 범위만 지원하고 NumLock에 따라 숫자가 이동 키로 바뀔 수 있습니다. 일반/넘패드 Enter와 좌우 modifier는 구분하지 않습니다. **Enqueue가 활성 상태여야** 하며 OS 전역 핫키·MIDI·Win 조합·한/영·한자·IME·미디어 키는 지원하지 않습니다. 미디어 장치는 F13~F24 같은 키로 설정하세요. 외부 프로세스·별도 UI 스레드의 플러그인 창은 보장 범위 밖입니다.
