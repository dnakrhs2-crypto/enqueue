...
2. 명령 행의 **학습** 또는 큐 인스펙터 기본 탭의 **입력 추가**를 누르고 키나 MIDI를 보냅니다. 첫 유효 후보는 고정됩니다. 바꾸려면 **다시 학습**을 누릅니다. 학습 중에는 키보드·MIDI 패닉을 포함한 실행을 억제하며 Esc는 취소만 합니다. 캡처 내부 규칙 필드로 포커스를 옮겨도 계속 학습하며 대상 큐 변경·탭 닫기·쇼 모드·위젯 파괴는 취소합니다.
...
6. 연결 뒤 입력을 준비합니다. **Note는 첫 누름부터 실행**하며 note-off 또는 velocity 0 note-on으로 해제합니다. 해제 없이 반복 on은 재실행하지 않습니다. **CC는 연결 뒤 첫 값은 기준값이라 실행되지 않음 — 페달·버튼은 노트 권장.** 학습·매핑·프로젝트 경계를 눌린 채 넘긴 입력은 놓아야 다시 실행됩니다. 장치·규칙 상태는 설정 탭·인스펙터·푸터에서 확인합니다. 과부하는 손실·100ms 초과 누계입니다.
