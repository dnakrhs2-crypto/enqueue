...
export class FakeHost extends EventEmitter {
  ...
  async start(appdata, language = "ko") {
    ...
    this.send({ event: "deviceDidConnect", device: "fake-mobile", deviceInfo: this.info.devices[0] });
    return this;
  }
  ...
  event(event, context, payload = {}, extra = {}) {
    ...
    this.send({ event, action: actionUUID, context, device: c.device, payload: { controller: "Keypad", coordinates: c.coordinates, isInMultiAction: false, resources: {}, settings: c.settings, state: c.state, ...payload }, ...extra });
  }
  appear(context, settings, column = 0) {
    this.contexts.set(context, { device: "fake-mobile", coordinates: { column, row: 0 }, settings, state: 0, visible: true, images: {}, titles: {} });
  ...
  disappear(context) { this.event("willDisappear", context); this.contexts.get(context).visible = false; }
  receivePlugin(m) {
    if (["setState", "setImage", "setTitle", "showAlert"].includes(m.event)) {
  ...
  async openInspector(context) {
    ...
    return { client, pi, request: (requestId = "pi1") => client.send(JSON.stringify({ event: "sendToPlugin", context: uuid, action: actionUUID, payload: { op: "getOptions", requestId } })) };
  }
  ...
  async close() {
    ...
    for (const [id, c] of this.contexts) if (c.visible && this.pluginSocket?.readyState === WebSocket.OPEN) this.disappear(id);
    if (this.pluginSocket?.readyState === WebSocket.OPEN) this.send({ event: "deviceDidDisconnect", device: "fake-mobile" });
