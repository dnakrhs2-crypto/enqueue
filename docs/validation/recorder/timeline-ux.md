...
마커 목록과 선택 위치는 `mm:ss.mmm`(1시간부터 `h:mm:ss.mmm`)로 표시하고, 위치 입력은 시간 또는 정수 샘플을 받는다. 추가 창에도 요청 시점의 위치를 표시한다.

마커 패널의 **텍스트로 내보내기**는 시간순 유튜브 챕터를 UTF-8(BOM 없음), CRLF로 저장한다. 녹화 잠금 중에도 사용할 수 있고 완료 안내를 상태줄에 남긴다.

마커 패널과 추가 창은 이름이 붙은 10색 견본을 키보드로도 선택할 수 있다. 패널에서는 색만 한 번의 편집으로 적용하며, 기존 프로젝트의 임의 색은 유지한다.
