...
| 버전 | 1.1.0 (manifest 1.1.0.0) |
| 태그 후보 | LiveMix, microphone, audio, streaming, effects |
| 지원 URL | https://곰튀김.com/livemix/#streamdeck-support |

## 설명

방송 중 Stream Deck에서 LiveMix의 마이크와 FX를 조작하세요. 개별 마이크나 전체 마이크를 켜고 끄고, 마이크·FX 뮤트그룹을 바꾸거나 같은 번호의 플러그인 그룹을 한 마이크 또는 전체 마이크에서 한꺼번에 바꿀 수 있습니다. LiveMix 창을 앞으로 가져오지 않아도 됩니다.
...
같은 Windows 10/11 x64 PC에서 LiveMix 0.6.0 이상과 Stream Deck 7.1 이상이 필요합니다. 새 플러그인 그룹 (전체 마이크) 키에만 LiveMix 0.10.0 이상이 필요하며 기존 여덟 동작은 0.6.0 이상에서 계속 사용할 수 있습니다. LiveMix 설정에서 ‘외부 제어 사용’을 켜세요. 실제 소리를 처리하려면 지원되는 ASIO 장치가 필요하지만, 오디오가 멈춰 있어도 제어 기능을 시험할 수 있습니다. Mobile 키는 Windows의 Stream Deck 앱을 통해 작동하며 다이얼은 Stream Deck +가 필요합니다. 플러그인은 한국어·영어를 지원하고 LiveMix 본체 UI는 현재 한국어입니다.

## 요구사항

- Windows 10/11 x64, LiveMix 0.6.0+, Stream Deck 데스크톱 앱 7.1+. 두 앱을 같은 Windows 사용자로 실행합니다.
- 플러그인 그룹 (전체 마이크) 키에만 LiveMix 0.10.0+가 필요합니다. 이전 버전에서는 업데이트 안내를 표시하고 명령을 보내지 않으며, 기존 여덟 동작은 그대로 사용할 수 있습니다.
...
| 플러그인 그룹 (전체 마이크) | LiveMix 0.10.0+. 해당 번호 그룹이 있는 모든 마이크에서 토글 / ON / OFF. 하나라도 ON이면 전부 OFF, 전부 OFF이면 전부 ON. 그룹 숫자, 모두 ON·모두 OFF·일부 ON과 켜진 수/전체 수를 표시합니다. |
