...
| Version | 1.1.0 (manifest 1.1.0.0) |
...
Control LiveMix microphones and FX from Stream Deck while you stream. Toggle a microphone or all microphones, mute assigned microphone and FX groups, and switch numbered plugin groups on one mic or every mic at once without bringing LiveMix to the front.
...
Requires LiveMix 0.6.0 or later and Stream Deck 7.1 or later on the same Windows 10/11 x64 PC. The new Plugin Group (All Mics) key requires LiveMix 0.10.0 or later; the other eight actions still support 0.6.0+. Enable External Control in LiveMix settings before use. A supported ASIO device is needed for audio processing, but not to test the controls. Keys support Stream Deck Mobile through the Windows desktop app; dials require Stream Deck +. The plugin includes English and Korean interfaces. LiveMix itself currently uses a Korean interface.

## Requirements

- Windows 10/11 x64; LiveMix 0.6.0+ and Stream Deck desktop app 7.1+ running as the same Windows user.
- Plugin Group (All Mics) requires LiveMix 0.10.0+. On older versions this key shows an update message and sends no command; the other eight actions remain available.
...
| Plugin Group (All Mics) | LiveMix 0.10.0+. Same numbered group across every mic that has it: any ON → all OFF, all OFF → all ON; explicit ON/OFF modes. Shows group digit, All/Some ON or All OFF and ON/total count. |
...
Copy only the three paragraphs under Description into the Console description field. The remaining sections supply requirement/action details and additional links. English is the primary submission copy.
