...
세션 ①-d P2 리뷰 반영(2026-09-20):

- 삭제된 GO 바인딩의 물리 관측과 기존 GO 토큰 유지 책임을 분리했다. 삭제 중 다른 포트의 CC region/down도 계속 갱신하고, 기존 토큰이 모두 해제되어도 물리적으로 눌린 관측은 복원까지 보존한다. 복원은 실행 없이 눌림만 이관한다. 상승 `127→62`·하강 `0→62`(low=60/high=64), A의 복원 전·후 해제와 Note GO 혼합을 검증한다.
- 공유 MPSC/64회 CAS 예약을 포트별 사전 할당 SPSC(일반 8,192칸 + 패닉 예약 512칸)로 대체했다. 포트별 콜백은 직렬 생산자이며 소비자는 포트를 순회해 각 포트의 순서를 보존한다. 콜백의 할당·잠금·OS 알림 없이 원자적 대기 건수로 notifier에 알린다. 용량 손실 합계 `capacityDropped`와 경합 손실 `contentionDropped`를 분리 노출하며, 후자는 예약 경합 경로 제거로 항상 0이다. 100ms 정책·일반/패닉 손실 세대는 유지하고, 연결 종료 시 해당 큐만 비운다. 같은 프로세스에서 32개 생산자의 전달을 32회 반복하며 포트별 용량·패닉 예약·재연결·60초 부하를 검증한다.

세션 ①-e 통합 리뷰·GUI 실측 반영(2026-09-20):

- 1: 삭제된 GO CC gate의 관측 runtime은 기존 GO 토큰이 모두 해제되어도 프로젝트 교체까지 보존한다. 삭제 중 새 토큰은 만들지 않으며 연결 종료·손실 때 해당 포트의 관측을 지운다. A 해제 뒤에 B가 처음 보내는 상승 `127→62`·하강 `0→62`를 복원하고 B의 실제 해제 전·후 GO 실행 횟수를 검증한다.
- 2: 학습 시작 당시 눌린 CC gate를 규칙별로 고정하고 각 규칙의 첫 해제까지만 기다린다. 같은 주소의 반대 gate가 새로 눌려도 대기를 이어받지 않으며 학습 중 명령·큐·패닉 억제는 유지한다.
- 3: 후보 외 포트를 포함해 활성 MIDI 연결이 종료되거나 입력이 유실되면 학습 전체를 취소한다. 확인할 수 없는 해제를 등록으로 간주하지 않고 세션 관측·등록 대기·캡처 토큰을 함께 폐기한다.
- 4: MainComponent 종료는 MIDI 수신 정지 → 캡처 취소 → 설정창 폐쇄·인스펙터 참조 해제 → MIDI 서비스 삭제 순서다. 실제 MainComponent와 설정 학습 종료 콜백으로 서비스 조회 수명을 검사한다.
- 5: 지정된 MIDI/UI 변경 파일의 비ASCII 리터럴을 검사하고 접기 헤더·장치 재지정 구분자·CC 제안 범위·CC 상세·큐 MIDI 툴팁을 명시적 UTF-8 변환으로 통일한다. 내부에서 변환하는 기존 헬퍼는 유지한다.
- 6: 포트는 장치 개방·시작 즉시 `연결`이다. `준비 대기`는 손실 뒤 새 유효 메시지 수신 전까지만이며 매핑 유무·CC 기준값과 무관하다. 푸터 요약도 포트 상태를 따르고 CC 기준값·해제·움직임 대기는 규칙 상태/툴팁에 표시한다.
- 7: 기본 페이지 MIDI는 핫키 옆 26px 한 줄이다. 비어 있으면 `MIDI: 미지정 / 입력 추가`, 지정 후에는 항목 선택·추가(+)·변경·삭제·상태를 표시한다. 창 1100px 미만에서만 다음 줄로 내리고 기본 페이지 최소 크기는 원래 860×232로 복원한다. 규칙 상세와 전체 상태는 툴팁에서도 확인한다.
- 앱 테마·100% 논리 좌표 확인: 창 1541px에서 기본 페이지 1509×232, 핫키 `(574,84,120,30)`, MIDI `(740,84,757,26)`이다. 창 1100px은 MIDI `(680,84,376,26)`, 1099px은 `(12,122,1043,26)`이다. 인스펙터 기준 페이지 원점은 `(1,81)`이며 230px 높이에서 미지정/지정 상태의 모든 MIDI 컨트롤이 보인다. 설정 명령 행 학습·인스펙터 핫키/MIDI 버튼은 JUCE 접근성 트리에서 확인했으나 외부 Windows UI Automation 클라이언트 재실측은 남는다.
- 검증: 1~7 수정 전 실패 확인(4는 종료 콜백 서비스 조회 시 `0xC0000005`). local Release Enqueue·EnqueueTests 빌드 성공, 신규 코드 경고 0. 기본 683개 시나리오·30,088개 검사 통과/실패 0, 76.926초. 별도 부하 18개 검사 통과/실패 0, 60.269초, 수신/전달 600,000건·실행 599,992회, 손실·stale·순서 오류 0, p95 1.550ms/p99 1.623ms/최대 2.581ms. 기존 JUCE·플러그인 경고는 유지하며 loopMIDI 및 실제 종료 예약 경로의 재실측은 수행하지 않았다.
- 세션 ①-f 최종 리뷰 반영(2026-09-20): 기본 페이지는 MIDI 줄바꿈·핫키 충돌 안내를 포함해 메모 최소 30px을 확보한다(넓은 창 232px 유지, 줄바꿈 236px·충돌 동반 258px). 준비 대기 툴팁의 상태 설명 뒤에 과부하 기록을 덧붙였고, 앱 내 설명서의 포트/규칙 상태·핫키 옆 배치·일반 모달의 패닉 예외를 README와 맞췄다.
